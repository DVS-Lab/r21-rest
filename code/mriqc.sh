...
# Run MRIQC for one participant.
#
# Usage:
#   bash code/mriqc.sh 189
#   bash code/mriqc.sh sub-189 --dry-run

usage() {
    cat <<'USAGE'
Usage: code/mriqc.sh SUBJECT [--dry-run]

Run MRIQC for one participant. SUBJECT may be written as 189 or sub-189.
USAGE
}
...
workdir="${MRIQC_WORK_DIR:-${scratchroot}/mriqc/sub-${sub}}"
...
nprocs="${MRIQC_NPROCS:-8}"
omp_nthreads="${MRIQC_OMP_NTHREADS:-4}"
mem_gb="${MRIQC_MEM_GB:-12}"
...
mkdir -p "$outputdir" "$workdir" "$mplconfigdir"
