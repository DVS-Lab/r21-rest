#!/usr/bin/env python3
"""Compile legacy or Smith09-plus-reward PPI randomise outputs."""
...
NEW_PPIS = ("dmn-ecn", "dmn-reward", "dmn-left-fpn", "dmn-right-fpn")
SUMMARY_FIELDS = (
    "analysis",
    "ppi",
    "network",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ppi-set",
        choices=("legacy", "all", *NEW_PPIS),
        default="legacy",
        help="legacy checks the original 10-map DMN x ECN analysis (default).",
    )
    ...
    parser.add_argument(
        ...
        help="Legacy/custom single-PPI directory; used only with --ppi-set legacy.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def copied_name(
    ...
    description = (
        f"Smith09{camel_label(ppi)}PpiComp{component:04d}"
...
def write_sidecar(
    ...
    payload = {
        "Description": "FSL randomise FWE-corrected 1-p map for a network-timeseries interaction",
        "TaskName": task,
...
def analysis_specs(
    args: argparse.Namespace, fsl_dir: Path
) -> list[tuple[str, str, Path, int, bool]]:
    if args.ppi_set == "legacy":
        return [
            (
                "dmn-ecn",
                "smith09_ppi-dmn-ecn",
                args.ppi_dir.resolve(),
                args.component or 11,
                False,
            )
        ]
    ppis = NEW_PPIS if args.ppi_set == "all" else (args.ppi_set,)
    return [
        (
            ppi,
            f"smith09-reward_ppi-{ppi}",
            fsl_dir / f"dual-regression_smith09-reward_denoised_ppi-{ppi}.dr",
            args.component or 12,
            True,
        )
        for ppi in ppis
    ]
...
def main() -> int:
    ...
    if args.component is not None and args.component < 1:
    ...
    specs = analysis_specs(args, fsl_dir)
    ...
    significant_count = copied_count = roi_value_count = roi_value_rows = 0
    tstat_count = corrp_count = 0

    for ppi, analysis, ppi_dir, component, network_in_prefix in specs:
        network = f"dmn-x-{ppi.removeprefix('dmn-')}"
        ...
        for contrast in CONTRASTS:
            ...
            randomise_dir = component_dir / "randomise"
            if network_in_prefix:
                randomise_dir = randomise_dir / f"network-{network}"
                ...
                prefix = randomise_dir / (
                    f"task-{args.task}_component-{component_padded}_stat-{args.map_type}_"
                    f"contrast-{contrast}"
                )
            tstats = {
    ...
    if args.ppi_set == "legacy":
        ...
        output_tsv = output_dir / f"task-{args.task}_smith09-reward_ppi_randomise_peak_summary.tsv"
    ...
    (output_dir / "README.md").write_text(
        ...
        "store `1-p`; values above 0.95 correspond to corrected p-values below 0.05. "
        "The current Smith09-plus-reward analyses use z-scored run-level timecourses "
        "and component 12 for DMN interactions with ECN, reward, left FPN, and right "
        "FPN. Significant maps are copied here with portable ROI-value TSVs.\n"
    )

    total_jobs = len(specs) * len(CONTRASTS)
    total_expected = total_jobs * len(DIRECTIONS) * len(methods)
    print(f"PPI analyses checked: {len(specs)}")
    print(f"Randomise jobs checked: {total_jobs}")
    print(f"t-stat images present: {tstat_count}/{total_jobs * len(DIRECTIONS)}")
    ...
    for path in sorted(missing_paths, key=str)[:40]:
        print(f"MISSING: {path}", file=sys.stderr)
    for error in validation_errors[:20]:
        print(f"ERROR: {error}", file=sys.stderr)
    for error in peak_errors[:20]:
        print(f"ERROR: {error}", file=sys.stderr)
    incomplete = bool(missing_paths or validation_errors or peak_errors)
