...
Options:
  ...
  --max-jobs N              Concurrent jobs. Default: MRIQC_MAX_JOBS. N=5 is an explicit override.
...
max_jobs="${max_jobs:-$MRIQC_MAX_JOBS}"
...
((${#subjects[@]})) || die "No participants selected for MRIQC."
...
check_batch_resources "MRIQC" "$active_jobs" "$MRIQC_NPROCS" "$mriqc_mem_mb" "$allow_oversubscribe" "$dry_run"
...
if is_truthy "$dry_run"; then
    info "Rendering MRIQC commands for ${#subjects[@]} participant(s)."
    for subject in "${subjects[@]}"; do
        "${SCRIPT_DIR}/run_mriqc_subject.sh" "${subject_flags[@]}" --participant "$subject"
    done
    exit 0
fi
...
batch_manifest_dir="${MANIFEST_ROOT}/mriqc"
ensure_dir "$batch_manifest_dir" "${LOG_ROOT}/mriqc"
batch_manifest="${batch_manifest_dir}/${stamp}.batch.tsv"
{
    printf 'participant\tcommand\n'
    for subject in "${subjects[@]}"; do
        printf '%s\t' "$subject"
        print_command "${SCRIPT_DIR}/run_mriqc_subject.sh" "${subject_flags[@]}" --participant "$subject"
    done
} > "$batch_manifest"
info "Recorded MRIQC batch manifest: $batch_manifest"
