...
1.5-IQR flags but does not exclude anyone. The earlier absolute-pairwise screen
generated by `select_qc_sensitivity_exclusions.py` is retained for provenance
but is not an adopted exclusion rule.
...
directions align across participants. In the N=27 data, there is no consistent
condition pattern for tSNR (`p=.58`), mean FD (`p=.75`), or high-motion
percentage (`p=.96`). No automatic data-quality exclusion is currently applied.
