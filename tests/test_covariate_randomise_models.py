...
class CovariateRandomiseModelTests(unittest.TestCase):
    ...
    def test_write_design_files_uses_intercept_plus_covariates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            design_con_text = design_con.read_text()
            self.assertIn("/ContrastName1\tmean_pos", design_con_text)
            self.assertIn("/ContrastName4\tcov_neg", design_con_text)
            self.assertIn("/NumContrasts\t4", design_con_text)
            self.assertIn("/RequiredEffect\t0.943\t0.943\t5.012\t5.012", design_con_text)
            self.assertIn("1\t0", design_con_text)
            self.assertIn("-1\t0", design_con_text)
            self.assertIn("0\t1", design_con_text)
            self.assertIn("0\t-1", design_con_text)
    ...
    def test_run_randomise_launcher_uses_shell_array(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("expected_outputs_exist()", text)
            self.assertIn('[[ "$contrast_count" == "4" ]]', text)
            self.assertIn('"${cmd[@]}"', text)
